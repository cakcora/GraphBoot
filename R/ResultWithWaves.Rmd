...
pu<-pu+ geom_line(size=1)+scale_y_continuous(limits=c(0,1.4),name=expression(paste(bar(x)," / ",mu)))
pu<-pu+ theme_bw()+theme(legend.position = c(0.8, 0.3),text = element_text(size=16)) 
...
ggsave(filename=paste(figFolder,"resultswaveUndirected1.eps",sep=""),plot=pu,width=5,height=3,unit="in")
ggsave(filename=paste(figFolder,"resultswaveUndirected1.png",sep=""),plot=pu,width=5,height=3,unit="in")
...
pd<-pd+ geom_line(size=1)+scale_y_continuous(name=expression(paste(bar(x)," / ",mu)))
