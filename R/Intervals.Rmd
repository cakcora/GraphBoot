...
m12<- ggplot(data[data$seed>10,],aes(x=seed,y=V1, color=dataset,group=dataset))+geom_line(size=2)+scale_x_continuous(name="seed")+theme(legend.position = c(0.8, 0.6),text = element_text(size=16))+scale_y_continuous(name="Relative interval length")
...
ggsave(filename=paste(figFolder,"intervals.png",sep=""),plot=m12,width=6,height=4,unit="in")
ggsave(filename=paste(figFolder,"intervals.eps",sep=""),plot=m12,width=6,height=4,unit="in")
...
m13<- ggplot(depdata,aes(x=wave,y=V1, color=aspect,group=aspect))+geom_line(size=2)+scale_x_continuous(name="wave",breaks=c(0,1,2))+theme(legend.position = c(0.8, 0.8),text = element_text(size=16))+scale_y_continuous(name="Relative interval length")
